				...
				<div class="row">
					<div class="col-xs-12">
						...
						<div class="row">
							...
							<div class="col-md-6">
								...
								<div class="row">
									<div class="col-md-12">
										<p>
											<a href="/dashboard/myactivities"
												class="btn btn-xs btn-warning text-right">My Activity
												Feed</a>
										</p>
									</div>
								</div>
									...
									<div class="col-md-12">
										<p>
											<a href="/dashboard/mypoints"
												class="btn btn-xs btn-blue text-right">My GlamPoints
												History</a>
										</p>
									</div>
								</div>
						...
						<ul class="nav nav-tabs nav-justified ">
							<li><a href="#activities" data-toggle="tab"><strong>My
										Feeds</strong></a></li>
						</ul>
								...
								<c:if test="${not empty activities }">
									<c:forEach items="${activities }" var="activity">
										<div class="media act-media">
											<a class="pull-left" href="/${activity.username }/"> <img
												class="media-object act-thumb" src="${activity.useravatar }"
												width="60px" />
											</a>
											<div class="media-body act-media-body">
												<strong>${activity.username }</strong> ${activity.actvtystr }
												<br /> <small class="text-muted">On
													${activity.actvtydttm }</small> <br />
												<c:if test="${activity.actvtytype == '_ATVY_LKBK_PH_ADD' }">
													<ul class="uploadphoto-list">
														<c:if test="${not empty activity.imgurl1 }">
															<li><a href="${activity.actvtyurl }"
																data-rel="prettyPhoto"><img
																	src="${activity.imgurl1 }"
																	class="img-responsive thumbnail" /></a></li>
														</c:if>
														<c:if test="${not empty activity.imgurl2 }">
															<li><a href="${activity.actvtyurl }"
																data-rel="prettyPhoto"><img
																	src="${activity.imgurl2 }"
																	class="img-responsive thumbnail" /></a></li>
														</c:if>
														<c:if test="${not empty activity.imgurl3 }">
															<li><a href="${activity.actvtyurl }"
																data-rel="prettyPhoto"><img
																	src="${activity.imgurl3 }"
																	class="img-responsive thumbnail" /></a></li>
														</c:if>
													</ul>
												</c:if>
												<c:if test="${activity.actvtytype != '_ATVY_LKBK_PH_ADD' }">
													<a href="${activity.actvtyurl }">${activity.actvtyurl }</a>
												</c:if>
											</div>
										</div>
										<!-- media -->
